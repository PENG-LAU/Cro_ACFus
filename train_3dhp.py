import argparse
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # set cuda
...
def evaluate(model, test_loader, n_frames):
    ...
    for data in tqdm(test_loader):
        ...
        # print('1:{0}'.format(out_target.shape))
        out_target[:, :, 14] = 0
        # out_target_t = gt_3D.clone().view(N, -1, 17, 3)

        # print('2:{0}'.format(out_target.shape))
        ...
        # print('3:{0}'.format(pred_out.shape))
        # pred_out_t = output_3D[:, pad].unsqueeze(1)
        # pred_out_t = denormalize(pred_out_t, seq)
        ...
        # -----------------------------------------------
        pck_test = pck_cal(pred_out, out_target).item()
        auc_test = auc_cal(pred_out, out_target).item()
        ...
        pck_sum.update(pck_test, N)
        auc_sum.update(auc_test, N)
    ...
    print(f'PCK avg: {pck_sum.avg * 100:.4f}')

    print(f'AUC avg: {auc_sum.avg * 100:.4f}')

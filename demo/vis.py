...
from model.MyModule import MyModule
...
def get_pose2D(video_path, output_dir):
    ...
    print('\nGenerating 2D pose...')
    keypoints, scores = hrnet_pose(video_path, det_dim=96, num_peroson=1, gen_output=True)
...
def turn_into_clips(keypoints):
    ...
    if n_frames <= 81:
        ...
        for start_idx in range(0, n_frames, 81):
            keypoints_clip = keypoints[:, start_idx:start_idx + 81, ...]
...
def get_pose3D(video_path, output_dir):
    ...
    args.mlp_ratio, args.act_layer = 2, nn.GELU
    ...
    args.num_heads, args.qkv_bias, args.qkv_scale = 4, False, None
    ...
    ## Reload 
    model = nn.DataParallel(MyModule(**args)).cuda()
    ...
    for idx, clip in enumerate(clips):
        ...
        for j, post_out in enumerate(post_out_all):
            ...
            str(('%04d'% (idx * 81 + j)))
            plt.savefig(output_dir_3D + str(('%04d'% (idx * 81 + j))) + '_3D.png', dpi=200, format='png', bbox_inches='tight')
            plt.close(fig)

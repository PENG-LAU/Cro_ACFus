...
def evaluate(args, model, test_loader, datareader, device):
    ...
    # 输出每个动作的损失
    print('15 actions: {0}'.format(action_names))
    print('P1 Action Err: {0}'.format(final_result))
    print('P2 Action Err: {0}'.format(final_result_procrustes))
    print('--------------------')
...
def train(args, opts):
    ...
    model.to(device)
    # b, c, t, j = 1, 3, 81, 17
    random_x = torch.randn((1, 81, 17, 3)).to(device=device)

    # print(f"model set:{next(model.parameters().device)}")
    # print(f"tochr set:{random_x.device}")
